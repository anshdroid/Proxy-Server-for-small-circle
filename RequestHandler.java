...
public class RequestHandler implements Runnable {
	...
	public void run() {
		...
		// Get Request from client
            ...
            clientDetail.add(timeStamp);
            Proxy.activeClients.put(clientIp, clientDetail);
            
            
            if(Proxy.isBClient(clientIp, clientName)) {
                System.out.println("Blocked client requesting...");
                return;
            }
		...
		// Check if site is blocked
                String tempIp = "";
                String tempString = urlString.substring(0, urlString.lastIndexOf(':'));
                
                try {
                    URL myurl = new URL(tempString);
                    tempIp = InetAddress.getByName(myurl.getHost()).getHostAddress();
                } catch (Exception e ) {
                    System.out.println("ip error while checking for the blocked.");
                }
		if(Proxy.isBlocked(tempIp,tempString)){
		...
		if(request.equals("CONNECT")){
			...
			if((file = Proxy.getCachedPage(urlString)) != null){
                ...
                //entry in log manager
                LocalDateTime nowDate = LocalDateTime.now();
                String year = Integer.toString(nowDate.getYear());
                String month = Integer.toString(nowDate.getMonthValue());
                String date = Integer.toString(nowDate.getDayOfMonth());
                String minute = Integer.toString(nowDate.getMinute());
                String fileName = "logManager/";
                String tempFile = tempString.replace(".", "0");
                tempFile = tempFile.replace(":", "0");
                tempFile = tempFile.replace("/","0");
                tempFile += ".txt";
                fileName += year+"/"+month+"/"+date+"/"+clientIp.replace(".", "0")+"/"+minute+"/" + tempFile;
                BufferedWriter fileToLogBw = null;
                ArrayList<String> logDetail = new ArrayList<>();
                logDetail.add(timeStamp);
                logDetail.add(tempIp);
                logDetail.add(tempString);
                logDetail.add(clientIp);
                logDetail.add(clientName);
                File fileToLog = new File(fileName);
                try {
                    if(!fileToLog.exists()) {
                    fileToLog.getParentFile().mkdirs();
                ...
                try{
                    FileOutputStream fos = new FileOutputStream(fileToLog);
				...
				System.out.println("Couldn't log: " + fileName);
				e.printStackTrace();
		} 
                Proxy.log.put(fileName, fileToLog); //data into log manager
	} 
	...
	private void sendNonCachedToClient(String urlString){
		...
		try{
			...
			if((fileExtension.contains(".png")) || fileExtension.contains(".jpg") ||
					fileExtension.contains(".jpeg") || fileExtension.contains(".gif")){
                            System.out.println("going for image file....");
				// Create the URL
	...
	private void handleHTTPSRequest(String urlString){
		...
		int port  = Integer.valueOf(pieces[1]);
                System.out.println("going for https request type...");
		try{
	...
	private void blockedSiteRequested(){
		try {
			...
			String line = "HTTP/1.0 403 Forbidden \r\n" +
					"User-Agent: ProxyServer/1.0\r\n" +
					"\r\n<html><head><title>Access Denied</title></head><body>Access Denied by Raj</body></html>";
